...
signal point_hovered(key, val, arg_other_point_data, sprite_node)
signal point_unhovered(key, val, arg_other_point_data, sprite_node)
...
func _input(event):
	if event is InputEventMouseMotion:
		...
		#on_data_hovered(hovered_data)

#func on_data_hovered(arg_data):
#	if arg_data == null:
#		on_data_unhovered()
#	else:
#		emit_signal()
#
#func on_data_unhovered():
#	pass
...
func _on_mouse_over(key, val, arg_other_point_data, sprite_node):
	current_mouse_over = key
	
	update()
	
	emit_signal("point_hovered", key, val, arg_other_point_data, sprite_node)
	#call_deferred("emit_signal", "point_hovered", key, val, arg_other_point_data, sprite_node)

func _on_mouse_out(key, val, arg_other_point_data, sprite_node):
	if current_mouse_over == key:
		current_mouse_over = null
		
		update()
		
		emit_signal("point_unhovered", key, val, arg_other_point_data, sprite_node)
		#call_deferred("emit_signal", "point_unhovered", key, val, arg_other_point_data, sprite_node)
...
#	_draw_chart_background(pointListObject)
	...
	if current_show_label & LABELS_TO_SHOW.Y_LABEL:
		...
		for ordinate_value in ordinate_values:
			...
			var position = Vector2(max(0, 6.0 -label.length()) * 9.5, (min_y + max_y - compute_y(ordinate_value) + (label_font.get_height() / 4)))
...
func compute_sprites(points_data, arg_other_point_data):
	var sprites = {}
...
func create_new_point(point_data, arg_other_point_data = null):
	_stop_tween()
	clean_chart()
	...
	if chart_type == CHART_TYPE.LINE_CHART:
		...
		current_data.push_back({
			...
			sprites = compute_sprites(point_data, arg_other_point_data)
		})
	elif chart_type == CHART_TYPE.PIE_CHART:
