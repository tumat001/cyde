...
var max_base_dmg_percent_amount : float
...
var base_dmg_modifier : PercentModifier
...
func _init(arg_base_unit_time_before_max : float).(StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__EFFECT_GIVER):
...
func _construct_effect():
	attk_speed_modifier = PercentModifier.new(StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__ATTK_SPEED_EFFECT)
	attk_speed_modifier.percent_based_on = PercentType.BASE
	
	attk_speed_effect = TowerAttributesEffect.new(TowerAttributesEffect.PERCENT_BASE_ATTACK_SPEED, attk_speed_modifier, StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__ATTK_SPEED_EFFECT)
	
	#####
	
	base_dmg_modifier = PercentModifier.new(StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__BASE_DMG_EFFECT)
	base_dmg_modifier.percent_based_on = PercentType.BASE
	
	base_dmg_effect = TowerAttributesEffect.new(TowerAttributesEffect.PERCENT_BASE_DAMAGE_BONUS, base_dmg_modifier, StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__BASE_DMG_EFFECT)
...
func _update_effect_modi():
	...
	var base_dmg_bonus = current_unit_time_before_max / base_unit_time_before_max * max_base_dmg_percent_amount
	base_dmg_modifier.percent_amount = base_dmg_bonus
	...
	for module in tower_affected.all_attack_modules:
		...
		if module.benefits_from_bonus_base_damage:
			...
			if tower_affected.main_attack_module == module:
				tower_affected._emit_final_base_damage_changed()
...
func _undo_modifications_to_tower(tower):
	var effect = tower.get_tower_effect(StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__ATTK_SPEED_EFFECT)
	if effect != null:
		tower.remove_tower_effect(effect)
	
	var base_dmg_effect = tower.get_tower_effect(StoreOfTowerEffectsUUID.SYN_CONFIDENTIALITY__BASE_DMG_EFFECT)
