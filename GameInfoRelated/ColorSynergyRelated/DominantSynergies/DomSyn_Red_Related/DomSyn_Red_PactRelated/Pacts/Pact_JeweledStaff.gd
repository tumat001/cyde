...
const TowerEffect_Red_JeweledStaff = preload("res://GameInfoRelated/ColorSynergyRelated/DominantSynergies/DomSyn_Red_Related/DomSyn_Red_PactRelated/PactEffects/TowerEffect_Red_JeweledStaff.gd")

signal on_current_ap_amount_changed(new_val)


var total_worth_per_100_to_ap_ratio : float
...
func _init(arg_tier : int, arg_tier_for_activation : int).(StoreOfPactUUID.PactUUIDs.JEWELED_STAFF, "Jeweled Staff", arg_tier, arg_tier_for_activation):
	...
	pact_icon = preload("res://GameInfoRelated/ColorSynergyRelated/DominantSynergies/DomSyn_Red_Related/DomSyn_Red_Assets/Pact_Icons/Pact_WealthStaff_Icon.png")


func _first_time_initialize():
	_update_level_related_vars()
	_update_curent_ap_amount()
...
func _update_curent_ap_amount():
	var total_worth = _calculate_total_player_worth()
	_current_ap_amount = round(((float(total_worth) / 100.0) * total_worth_per_100_to_ap_ratio) * 1000) / 1000
	...
	var ins_for_ap = []
	ins_for_ap.append(OutcomeTextFragment.new(TowerStatTextFragment.STAT_TYPE.ABILITY_POTENCY, -1, "ability potency", _current_ap_amount))
	
	interpreter_for_ap.array_of_instructions = ins_for_ap
	
	# INS END
	good_descriptions[1] = ["Current bonus: |0|", [interpreter_for_ap]]
	emit_signal("on_description_changed")
	
	emit_signal("on_current_ap_amount_changed", _current_ap_amount)
...
func _attempt_add_effect_to_tower(tower : AbstractTower):
	if !tower.has_tower_effect_uuid_in_buff_map(StoreOfTowerEffectsUUID.RED_PACT_JEWELED_STAFF_EFFECT):
		var effect = TowerEffect_Red_JeweledStaff.new(self)
...
func _remove_effect_from_tower(tower : AbstractTower):
	var effect = tower.get_tower_effect(StoreOfTowerEffectsUUID.RED_PACT_JEWELED_STAFF_EFFECT)
