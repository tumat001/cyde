...
const orange_tower_count_min_for_offerable : int = 2
...
func _init(arg_tier : int, arg_tier_for_activation : int).(StoreOfPactUUID.PactUUIDs.ORANGE_IDENTITY, "Orange Identity", arg_tier, arg_tier_for_activation):
	...
	good_descriptions = [
		["|0| benefit from the |1|.", [plain_fragment__red_tower, plain_fragment__orange_synergy]],
		["The |0| is always active.", [plain_fragment__orange_synergy]]
	]
	...
	pact_icon = preload("res://GameInfoRelated/ColorSynergyRelated/DominantSynergies/DomSyn_Red_Related/DomSyn_Red_Assets/Pact_Icons/Pact_OrangeIdentity_Icon.png")


func _first_time_initialize():
	sv_modi_id = game_elements.EnemyManager.SVModiferIds.DOM_SYN_RED__ORANGE_IDENTITY
	syn_to_always_activate_modi_id = game_elements.synergy_manager.SynergiesToAlwaysActivateModiIds.DOM_SYN_RED__ORANGE_SYNERGY__ORANGE
	tower_benefit_as_if_modi_id = AbstractTower.BenefitFromSynAsIfHavingColorsModiIds.DOM_SYN_RED__ORANGE_SYNERGY__ORANGE
...
func _apply_extra_difficulty__and_syn_relateds():
	...
	game_elements.synergy_manager.add_dominant_synergy_id_to_always_activate(syn_to_always_activate_modi_id, TowerDominantColors.SynergyId.ORANGE)
...
func _tower_to_benefit_from_pact(tower : AbstractTower, arg_update_syn : bool = true):
	if tower.is_benefit_from_syn_having_or_as_if_having_color(TowerColors.RED) and !tower.has_modi_id_benefit_from_syn_as_if_having_color(tower_benefit_as_if_modi_id):
		tower.add_benefit_from_syn_as_if_having_color(tower_benefit_as_if_modi_id, TowerColors.ORANGE, arg_update_syn)
		#tower.call_deferred("add_benefit_from_syn_as_if_having_color", tower_benefit_as_if_modi_id, TowerColors.ORANGE, arg_update_syn)
...
func is_pact_offerable(arg_game_elements : GameElements, arg_dom_syn_red, arg_tier : int) -> bool:
	...
	if arg_dom_syn_red._x_identity_stage_round_index_used_for_cals__8_rounds != curr_stageround_index:
		#print("curr index calculating - ORANGE: %s. %s" % [curr_stageround_index, arg_dom_syn_red._x_identity_stage_round_index_used_for_cals__8_rounds])
	...
	if first_bool_check:
		...
		for tower in all_towers:
			if tower.is_benefit_from_syn_having_or_as_if_having_color(TowerColors.ORANGE):
				...
				if tower_count <= orange_tower_count_min_for_offerable:
					return true
