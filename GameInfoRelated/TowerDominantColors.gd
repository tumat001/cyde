...
const CYDE_SynergyID__Confidentiality = "%s%s" % [CYDE_HeaderIDName, "Confi"]
const CYDE_SynergyID__Integrity = "%s%s" % [CYDE_HeaderIDName, "Integ"]
const CYDE_SynergyID__Availability = "%s%s" % [CYDE_HeaderIDName, "Availa"]
...
const all_dom_synergy_ids_from_Syn_TD_base_game : Array = [
	...
	CYDE_SynergyID__Confidentiality,
	CYDE_SynergyID__Integrity,
	CYDE_SynergyID__Availability,
]
...
func _on_singleton_initialize():
	...
	[CydeSyn_Integrity],
	[
		["The first 4 |0| to escape deal no damage.", [plain_fragment__enemies]],
		["And gain 4 shield per |0|.", [plain_fragment__end_of_round]],
		["And |0| are |1| the first time they reach near the exit.", [plain_fragment__enemies, plain_fragment__knocked_back]]
	],
	...
	var plain_fragment__tier_2_gold_amount = PlainTextFragment.new(PlainTextFragment.STAT_TYPE.GOLD, "2 gold")
	var plain_fragment__tier_1_gold_amount = PlainTextFragment.new(PlainTextFragment.STAT_TYPE.GOLD, "5 gold")
	
	
	var plain_fragment__availability_towers = PlainTextFragment.new(PlainTextFragment.STAT_TYPE.TOWER, "Accessibility Tower(s)")
	var plain_fragment__on_round_end = PlainTextFragment.new(PlainTextFragment.STAT_TYPE.ON_ROUND_END, "on round end")
	
	var plain_fragment_tier_2_availability_tower_count = PlainTextFragment.new(PlainTextFragment.STAT_TYPE.TOWER, "1 Accessibility Tower")
	var plain_fragment_tier_1_availability_tower_count = PlainTextFragment.new(PlainTextFragment.STAT_TYPE.TOWER, "2 Accessibility Towers")
	...
	synergies = {
		synergy_id_to_syn_name_dictionary[CYDE_SynergyID__Confidentiality] : confidentiality_syn,
		
		synergy_id_to_syn_name_dictionary[CYDE_SynergyID__Integrity] : integrity_syn,
		
		synergy_id_to_syn_name_dictionary[CYDE_SynergyID__Availability] : availability_syn,
	}
